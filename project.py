...
def display_characters(character_list, display_type): 
    if display_type == "list":      #checking display type wanted
        ...
        char_list = [character for character in character_list if character[2] == "h"]      #adding the heroes to a new list
    else:
        char_list = [character for character in character_list if character[2] == "v"]      #adding the villains to a new list
...
def add_character(character_list, name, secret_id, is_hero):
    ...
    if Flag == True:
        character = [name, secret_id, is_hero, 0, 0, 0, 0, 100]         #the health will be 100 in the initial stage
        character_list.append(character)        #adding the item to the list
        write_to_file("new_characters.txt", character_list=character_list)      #saving the updated value
...
def remove_character(character_list, name):     #function to remove specific character
    ...
    for character in character_list:
            ...
            if lname != name:
                new_character_list.append(character)        #appending the character excluding the selected one
    write_to_file("new_characters.txt",character_list=new_character_list)       #saving the updates
...
def display_highest_battles_won(character_list):
    ...
    for character in character_list:
        ...
        if no_won > highest_won or (no_won == highest_won and no_battles < low_battle):     # checking two conditions 
            highest_won_character = character       #assigning the value 
            highest_won = no_won
            low_battle = no_battles

    if highest_won_character:
        name, _, _, _, no_won, _, _, _ = highest_won_character      #taking the name and number of victory
...
def do_battle(character_list, opponent1_pos, opponent2_pos):
    ...
    opponent1 = character_list[opponent1_pos]       #getting the character using the index number
    ...
    opponent1_health = opponent1[7] - opponent1_damage      #minusing the random number from the character health
    opponent2_health = opponent2[7] - opponent2_damage
    if opponent1_health <0:     #if the present health is below zero, setting that to zero
        opponent1_health = 0
    if opponent2_health<0:
        opponent2_health = 0
    print(f"{'':<2}> {opponent1[0]} - Damage: {opponent1_damage} - Current health: {opponent1_health}")     #printing the damage and remaining health
    ...
    write_to_file("new_characters.txt", character_list=character_list)      #writing to file
...
def sort_by_health(character_list):
    ...
    for i in range(n-1):        #i loop from the index 0 to the 2nd last element.
        for j in range(0, n-i-1):       #get health and battles fought for the current and next characters
            ...
            if health_of_1 < health_of_2 or (health_of_1 == health_of_2 and battles_completed1 < battles_completed2):
                sorted_list[j], sorted_list[j + 1] = sorted_list[j + 1], sorted_list[j]     #swapping the values
...
char = read_file("characters.txt")      #reading characters.txt and converting into list format
write_to_file("new_characters.txt",character_list=char)     #converting and adiing the list to new_character.txt
...
while Game == True:     #checking the game status 
    ...
    if display_type in display_char: #if the command in the list the function will be called
        display_characters(character_list, display_type)
    elif display_type == "quit":        #quiting the game and writing the present list into new_characters.txt
        Game = False
        write_to_file("new_characters.txt", character_list)
    elif display_type == "search":
        name = input("Please enter name: ")  
        result = find_character(character_list, name)  #passing the list and name to the function
        ...
        if result != -1:
            ...
            write_to_file("new_characters.txt", character_list=character_list) #saving the updates
        else:
            print(f"{name} is not found in character (heroes and villains) list.")
        ...
        name = input("Please enter name: ")
        result = find_character(character_list, name)       #checking the input name is present 
        if result != -1:        #-1 means that the input name is not exist
            result = remove_character(character_list,name)      #calling the function to remove the character 
        else:
            print(f"{name} is not found in characters.")
    elif display_type == "high":
        display_highest_battles_won(character_list=character_list)      #to find out the character with highest battle won
        ...
        while first == -1:      #if the value is changed the while loop will break
            ...
            first_index = find_character(character_list, name1)     #checking the name exist
            if first_index == -1:
                ...
                character_list[first_index][3] += 1     #increasing the battle total of oppenent one
        while second == -1:
            ...
            if second_index == -1:
                ...
                second = second_index
                character_list[second_index][3] += 1        #increasing the battle total of oppenent one
        ...
        while i<round:      #to get through every round
            print(f"\nRound: {i+1}")        
            result = do_battle(character_list=character_list,opponent1_pos=first_index,opponent2_pos=second_index)      #passing the indexes and list to battle function
            character_list = read_file("new_characters.txt")        #to update the details
            if character_list[first_index][7] == 0 and character_list[second_index][7]!=0:      #checking any of the characters health is zero 
                print(f"\n{'':<3}-- {character_list[first_index][0]} has died! :( \n")      #if it's zero we assume that the character is died
                winner = character_list[second_index][0]        #setting the other character as winner
                ...
                i = round
            elif character_list[second_index][7] == 0 and character_list[first_index][7] == 0:      #if the both characters are dead we stop the battle
                i = round   
            i += 1
        print(f"\n{'':<3}-- End of battle --")
        if winner: #cheking winner have a valid name
            print(f"{'':<3}** {winner} wins! **")
            if character_list[second_index][7] == 0: #updation the battle status to the respective characters
            ...
            if character_list[first_index][7] == character_list[second_index][7]: #conditions to determine the final winner
                ...
                character_list[second_index][5] += 1
        write_to_file("new_characters.txt", character_list=character_list)      #updating the list
    elif display_type == "health":
        result = sort_by_health(character_list)     #sorting the list  
        display_characters(character_list=result, display_type="list")      #displaying the list
    else:
        print("Not a valid command - please try again..")       #if the command not in any of these
